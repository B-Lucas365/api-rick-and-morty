...
export const Character = ({characterId, character, characterName, selectCharacter, cardSmall, favorite, addNewFavorite, removeFavorite}: CharacterProps) => {
        ...
        <Card id="card-character" small={cardSmall} onClick={selectCharacter ? () => selectCharacter(characterId) : undefined}>
            {!favorite && <div className="block-favorite block-want-favorite" onClick={() => addNewFavorite(characterId !== undefined ? characterId : 0)}><BsStar color={"#FFF"} /></div>}
            {favorite && <div className="block-favorite block-already-favorite" onClick={() => removeFavorite(characterId !== undefined ? characterId : 0)}><BsStarFill size={12} color={"#FFD600"} /></div>}
